# -*- coding: utf-8 -*-
"""
File: testDominion1.py
Purpose: Create test scenario in which the game over condition is incorrect and the game ends when 2 piles of cards are gone
...
def num_stacks_gone(supply):
    num_stacks_gone = 0
    for stack in supply:
        if len(supply[stack]) == 0:
            num_stacks_gone += 1
    return num_stacks_gone
...
supply = create_card_supply(card_box, 10, num_players, num_victory, num_curse)
...
turn  = 0
while num_stacks_gone(supply) < 2:
    turn += 1    
    print("\r")   
